...
import { clusterColors, colorToOklch, displayName, distance, normalize } from "../skill/scripts/normalize.js";
...
test("guide accepts legacy role labels and keeps Neo-Brutalism shadows accurate",()=>{ const d=normalize(base); const styled={...d,style:{primary:"Neo-Brutalism" as const,secondary:null,descriptors:["direct","bold","tactile"],thesis:"以清晰的边界和直接的色彩建立视觉秩序。阴影与描边共同传达明确的触感。"},screenshot:"screenshot.png"}; styled.tokens.colors=[{role:"背景 bg",hex:"#fff4dd",stability:"L1",freq:.2},{role:"正文 text",hex:"#000000",stability:"L1",freq:.2},{role:"分隔面 surface",hex:"#ffd84d",stability:"L2",freq:.2},{role:"强调 accent",hex:"#ff5c2b",stability:"L1",freq:.2},{role:"边框 border",hex:"#877b6b",stability:"L1",freq:.2}]; styled.tokens.shadow={weight:"有",md:"rgb(38, 32, 26) 3px 3px 0px 0px"}; const output=guide(styled); assert.match(output,/## 语义色彩\n/); assert.match(output,/\| 表面 \| #ffd84d \|/); assert.match(output,/## CSS/); assert.match(output,/3px 3px 0px 0px/); assert.equal(output.includes("避免重阴影"),false); assert.equal(output.includes("不要引入装饰字体"),false); assert.equal(output.includes("CTA"),false); });
