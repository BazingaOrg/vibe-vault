...
import { defaults, fontFamilyName, fontStackSummary, foregroundFor, previewBackdropFor, splitFontStack } from "../assets/theme.js";
...
test("normalizes a versioned public record without control-specific fields",()=>{ const d=normalize(base); assert.equal(d.schemaVersion,2); assert.equal(d.id,"example-com"); assert.ok(d.tokens.colors.some(c=>c.role==="背景 bg")); assert.ok(d.theme["--tk-bg"]); assert.equal(d.tokens.space.unit,8); assert.ok(clusterColors(base).length>=3); assert.equal("rawPath" in d,false); assert.equal(Object.keys(d.theme).some((name)=>name.startsWith("--tk-btn-")),false); assert.equal(Object.keys(defaults).some((name)=>name.startsWith("--tk-btn-")),false); assert.equal(guide({...d,style:{primary:"Minimalism",secondary:null,descriptors:["calm","clean","precise"],thesis:"保持清晰而克制的视觉层级，让内容和留白共同建立稳定节奏。避免用装饰取代信息优先级。"},screenshot:"screenshot.png"}).includes("--tk-btn-"),false); });
...
test("font stack summaries are quote and escape aware without changing source values",()=>{
 ...
 assert.equal(fontFamilyName(yeguozi),"Helvetica Neue"); assert.equal(fontFamilyName(""),"暂未识别");
...
test("domain ids and strict judgments",()=>{ assert.equal(siteId("https://www.Stripe.com/pay"),"stripe-com"); assert.throws(()=>validateJudgment({primary:"Invented",secondary:null,descriptors:[],thesis:"x"})); assert.throws(()=>validateJudgment({primary:"Minimalism",secondary:"Minimalism",descriptors:["calm","clean","premium"],thesis:"保持清晰而克制的层级，并让核心信息优先出现。使用稳定的留白建立节奏，避免把页面塞满装饰。"})); assert.throws(()=>validateJudgment({primary:"Minimalism",secondary:null,descriptors:["calm","calm","premium"],thesis:"保持清晰而克制的层级，并让核心信息优先出现。使用稳定的留白建立节奏，避免把页面塞满装饰。"})); assert.throws(()=>validateJudgment({primary:"Minimalism",secondary:null,descriptors:["calm","clean","premium"],thesis:"保持清晰而克制的层级，并让核心信息优先出现。使用稳定的留白建立节奏，避免把页面塞满装饰。",extra:true})); assert.equal(validateJudgment({primary:"Minimalism",secondary:null,descriptors:["calm","clean","premium"],thesis:"保持清晰而克制的层级，并让核心信息优先出现。使用稳定的留白建立节奏，避免把页面塞满装饰。"}).primary,"Minimalism"); });
...
test("chooses repeated real shadows and rejects isolated control effects",()=>{
 ...
 const isolated:RawExtraction={...base,samples:[base.samples[0],sample("0 6px 20px rgba(0,0,0,.2)","button",80,30)]};
 assert.equal(normalize(isolated).tokens.shadow.md,"none");
});
...
test("collect samples a local fixture, archives stale judgment, and preserves manual evidence", async()=>{ const root=await mkdtemp(path.join(tmpdir(),"vibe-vault-")); try { const fixture=pathToFileURL(path.join(path.dirname(fileURLToPath(import.meta.url)),"fixtures/collector.html")).href; const {raw,dir,status}=await collect(fixture,root); assert.ok(raw.samples.length>0); assert.notEqual(status,"failed"); assert.equal(raw.warnings.some(w=>w.includes("采集不完整")),false); assert.ok(raw.samples.some((sample)=>typeof sample.letterSpacing==="number"&&typeof sample.borderWidth==="number"&&typeof sample.opacity==="number")); const { access, writeFile } = await import("node:fs/promises"); await access(path.join(dir,"screenshot.png")); await access(path.join(dir,"evidence","manifest.json")); await access(path.join(dir,"evidence","primary-top.png")); await writeFile(path.join(dir,"judgment.json"),"{}\n"); await writeFile(path.join(dir,"evidence","manual-note.json"),"{}\n"); await collect(fixture,root); await assert.rejects(access(path.join(dir,"judgment.json"))); await access(path.join(dir,"judgment.previous.json")); await access(path.join(dir,"evidence","manual-note.json")); } finally { await rm(root,{recursive:true,force:true}); } });
