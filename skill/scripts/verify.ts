...
export async function fidelity(screenshot: string, raw: RawExtraction, tokens: Tokens) {
  ...
  d.sort((a,b)=>a-b); const coverage=d.filter(x=>x<=.06).length/Math.max(1,d.length); return { coverage:Number(coverage.toFixed(3)), eligibleRatio:Number((eligible/Math.max(1,total)).toFixed(3)), mean:Number((d.reduce((a,b)=>a+b,0)/Math.max(1,d.length)).toFixed(3)), p95:Number((d[Math.floor(d.length*.95)]??0).toFixed(3)) };
}
